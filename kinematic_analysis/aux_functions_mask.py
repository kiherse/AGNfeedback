...
def fit_continuum(wave,flux,wlimits):
    ...
    # #for only one region           
    #mask_cont =  ((wave >= wlimits[0]) & (wave <= wlimits[1]))
...
def write_output_fit(wave_reg,cont_reg_model,line_combo_fit,line_combo_error,chi2_fit,rms_fit,noise,\
    flux_norm=1.,logfile=None,verbose=True):

    if (logfile != None):
        f=open(logfile,mode='w')
        f.write(f' Chi2= {chi2_fit:.3e}; Npts={len(wave_reg):4d}; RMS={rms_fit*flux_norm:.3e}; \
            Noise= {noise*flux_norm:.3e}\n')
    if line_combo_fit.n_submodels > 1:
        ...
        for submod,submod_error in zip(line_combo_fit,line_combo_error):
            ...
            if hasattr(submod,'ftype'):
                submod_ftyp = submod.ftype
            else:
                submod_ftyp = 'Gaussian1D'   
            if submod_ftyp == 'Lorentz1D':
                (submod_center,submod_center_error)=(submod.x_0,submod_error.x_0)
                submod_flux, submod_flux_error = calc_lorentz_flux(submod,flux_norm=flux_norm,Errpars=submod_error)
                submod_fwhm,submod_fwhm_error=calc_lorentz_fwhm(submod,velocity=True,Errpars=submod_error)
                submod_cont = cont_reg_model[np.argmin(np.abs(wave_reg-submod.x_0))]
            if submod_ftyp == 'Voigt1D':
                (submod_center,submod_center_error)=(submod.x_0,submod_error.x_0)
                submod_flux, submod_flux_error = calc_voigt_flux(submod,flux_norm=flux_norm,Errpars=submod_error)
                submod_fwhm,submod_fwhm_error=calc_voigt_fwhm(submod,velocity=True,Errpars=submod_error)
                submod_cont = cont_reg_model[np.argmin(np.abs(wave_reg-submod.x_0))]
            if submod_ftyp == 'Gaussian1D':
                (submod_center,submod_center_error)=(submod.mean,submod_error.mean)
            ...
            tab_row.append((submod.name,submod_center,submod_center_error,\
            ...
            if verbose:
                ...
                print(f' Center= {submod_center.value:.2f} +/- {submod_center_error.value:.2f}')
            ...
            if (logfile != None):
                f.write(submod.name + '\n')
                f.write(f' Center= {submod_center.value:.2f} +/- {submod_center_error.value:.2f} \n')
        ...
        tab_row=(submod.name,submod_center,submod_center_error,\
            submod_flux,submod_flux_error,submod_fwhm,submod_fwhm_error,\
            submod_eqw,submod_eqw_error)
        if verbose:
            print(submod.name)
...
def perform_fit_cont_plot(wave,flux,mask_reg,wlim_reg,wlim_cont,wave_O3r,wave_O3b,\
          line_combo,Nsimul=1000,flux_norm=1.,plotfile=None,logfile=None,tgt_name=None,\
          label_cont='Continuum',label_model='Model',tab_entry=None,figsize=(7,8),plot_subtracted=False,\
          xlabel=r"Wavelength [$\AA$] (Obs. frame)",fwhm_instr=None):

    # ajuste del continuo
    poly_cont,noise=fit_continuum(wave,flux,wlim_cont) 

    flux_reg = flux[mask_reg]
    wave_reg = wave[mask_reg]
    ...
    rms_fit = np.std(flux_reg_nocont - line_combo_mod)
    chi2_fit = np.sum((flux_reg_nocont - line_combo_mod)**2/noise**2)

    tab_entry = write_output_fit(wave_reg,cont_reg_model,line_combo_fit,line_combo_error,\
        chi2_fit,rms_fit,noise,flux_norm=flux_norm,logfile=logfile)

    if (plotfile != None):
        #ylabel1=r'$F_\lambda$ \n $[10^{-15}\ erg\ s^{-1}\ cm^{-2}\ \AA^{-1}]$'
        ylabel1='$F_\lambda$ $[10^{-15}\ erg\ s^{-1}\ cm^{-2}\ \AA^{-1}]$'
        ylabel2=r'$F_\lambda-F_\lambda^{mod} $'
        norm_plot = 1.e-15
        mask_plot = (wave > wlim_reg[0]) & (wave < wlim_reg[1])
        wave_plot = wave[mask_plot]
        flux_plot = flux[mask_plot]
        cont_plot = poly_cont(wave_plot)
        line_combo_plot = line_combo_fit(wave_plot) 
        f, (ax1, ax2) = plt.subplots(2, 1,figsize=(11,7), gridspec_kw={'height_ratios': [3, 1]}, sharex=True)   
        ax1.step(wave_plot,flux_plot*flux_norm/norm_plot,label='Observed',where='mid',color='gray',linewidth=3)
        # plot the fit to the continuum and mark the region used 
        ax1.plot(wave_plot,cont_plot*flux_norm/norm_plot,label='Stellar Model',\
                 color='burlywood',linestyle='dashdot',linewidth=1)
        mask_wlim_cont = np.array([np.argmin(abs(wave-wlim_cont[0][0])),\
                                   np.argmin(abs(wave-wlim_cont[0][1])),\
                                   np.argmin(abs(wave-wlim_cont[1][0])),\
                                   np.argmin(abs(wave-wlim_cont[1][1]))])
        wave_cont_bar = wave[mask_wlim_cont]
        flux_cont_bar = poly_cont(wave_cont_bar)
        ymin_cont_bar = flux_cont_bar*0.8*flux_norm/norm_plot
        ymax_cont_bar = flux_cont_bar*1.2*flux_norm/norm_plot
        # plot the model fit adding the continuum
        ax1.plot(wave_plot,(cont_plot+line_combo_plot)*flux_norm/norm_plot,label='Full Model',linewidth=2)
        ax1.axvline(centroid_4959/u.Angstrom,linestyle='--',color='gray')
        ax1.axvline(centroid_5007/u.Angstrom,linestyle='--',color='gray')
        # plot the continuum subtracted spectrum, add a pedestal 
        base_cont = np.median(cont_reg_model)*flux_norm/norm_plot*0.5
        if (plot_subtracted):
            ax1.step(wave_reg,(flux_reg-cont_reg_model)*flux_norm/norm_plot+base_cont,\
                label='Stellar sub',linewidth=1,where='mid')
        # plot individual model components if more than one, add a pedestal 
        if line_combo_fit.n_submodels > 1:
            for submod_fit in line_combo_fit:
                ax1.plot(wave_plot,submod_fit(wave_plot)*flux_norm/norm_plot+base_cont,label=submod_fit.name,\
                    linestyle='--')
        else:        
            ax1.plot(wave_plot,line_combo_plot*flux_norm/norm_plot+base_cont,label=line_combo_fit.name,\
                linestyle='--')  
        ...
        ax2.step(wave_plot,(flux_plot-cont_plot-line_combo_plot)*flux_norm/norm_plot,label='Residual',where='mid')
        ax2.axvspan(6863,6913,color='green',alpha=0.2)
        ax2.set_xlabel(xlabel,fontsize=16)
        ax2.set_ylabel(ylabel2,fontsize=16)
        ax2.set_xlim(wlim_reg[0]/u.Angstrom,wlim_reg[1]/u.Angstrom)
        ax2.tick_params(axis='both', labelsize=12)
        f.subplots_adjust(hspace=0)
        f.suptitle(tgt_name,y=0.92,fontsize=18)
        f.savefig(plotfile,bbox_inches='tight')

    return line_combo_fit, line_combo_error, tab_entry, chi2_fit, blueshift, eblueshift, nonpar_fit, nonpar_error, bestfit_outflow, errorfit_outflow,  bestfit_outflow_blue, errorfit_outflow_blue, bestfit_outflow_red, errorfit_outflow_red, params_simul, params_simul_nonpar

def perform_nonpar_plot(tgt_name,wave_reg,line_combo_fit,nonpar_fit,flux_norm):
    
    line_combo_fit_5007 = None
    for c in line_combo_fit:
        if '5007' in c.name:
            if isinstance(line_combo_fit_5007,Model):
                line_combo_fit_5007 = line_combo_fit_5007 + c
            else:
                line_combo_fit_5007 = c
                
    wave_ext = np.linspace(min(wave_reg),max(wave_reg),1000)
    flux_ext = line_combo_fit_5007(wave_ext)

    plt.figure(figsize=(9,7))
    xlabel=r"Velocity [km $s^{-1}$]"
    ylabel='$F_\lambda$ $[10^{-15}\ erg\ s^{-1}\ cm^{-2}\ \AA^{-1}]$'
    plt.plot(2.99e5*(wave_ext.value/nonpar_fit[0]-1),line_combo_fit_5007(wave_ext)*(flux_norm*u.A*u.cm*u.cm*u.s/u.erg)/1.0e-15,linewidth=3)
...
def calc_lorentz_fwhm(lorentz,velocity=True,Errpars=None):
    #print(gaussian)
    ## it should be done first the assignment and then numpy operations, otherwise 
    ## units are lost
    fwhm = lorentz.fwhm
    if velocity:
        fwhm *= (cspeed/lorentz.x_0)
    if Errpars:
        fwhm_err = Errpars.fwhm
        if velocity:
            fwhm_err *= (cspeed/lorentz.x_0)
        return fwhm, fwhm_err
    return fwhm

def calc_lorentz_flux(lorentz,flux_norm=1,Errpars=None):
    ## Flux of Lorentz = pi * Amplitude * FWHM/2
    flux = np.pi*lorentz.fwhm/2*flux_norm*lorentz.amplitude
    if Errpars:
        flux_err = np.pi/2*flux_norm*\
            np.sqrt(lorentz.fwhm**2*Errpars.amplitude**2+\
            lorentz.amplitude**2*Errpars.fwhm**2)
        return flux, flux_err   
    return flux

def calc_voigt_fwhm(voigt,velocity=True,Errpars=None):
    #print(gaussian)
    ## it should be done first the assignment and then numpy operations, otherwise 
    ## units are lost
    fwhm = np.sqrt(voigt.fwhm_L**2+voigt.fwhm_G**2)
    if velocity:
        fwhm *= (cspeed/voigt.x_0)
    if Errpars:
        fwhm_err = np.sqrt(Errpars.fwhm_L**2+Errpars.fwhm_G**2)
        if velocity:
            fwhm_err *= (cspeed/voigt.x_0)
        return fwhm, fwhm_err
    return fwhm

def calc_voigt_flux(voigt,flux_norm=1,Errpars=None):
    ## Flux of Lorentz = pi * Amplitude * FWHM/2
    flux = np.pi*voigt.fwhm_L/2*flux_norm*voigt.amplitude_L
    if Errpars:
        flux_err = np.pi/2*flux_norm*\
            np.sqrt(voigt.fwhm_L**2*Errpars.amplitude_L**2+\
            voigt.amplitude_L**2*Errpars.fwhm_L**2)
        return flux, flux_err   
    return flux
